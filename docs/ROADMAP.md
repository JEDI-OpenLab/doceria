...
- ✅ **Ingestion PDF/DOCX robuste** : ILaaS renvoie **502 sur tout upload PDF** → Doceria **extrait le texte EN LOCAL** (pdf.js/mammoth via `read_file` + `uploadFileSmart`) et l'envoie en `.md` (qui passe). **Toast d'upload global** (progression + erreur, visible rail replié / glisser-déposer ; ✕ + auto-fermeture sur succès). — **fait**.
- ⏳ **OCR des PDF scannés — REPORTÉ** : ILaaS **n'a pas de modèle OCR** au catalogue (vérifié en live 2026-06 : `bge-m3`, `bge-reranker-v2-m3`, `gpt-oss-120b`, `mistral-small-3.2-24b`) → `/v1/ocr` inutilisable (422 « Wrong model type »). Code OCR **retiré** ; un PDF scanné affiche « non importable ». **À reprendre** quand ILaaS proposera un modèle OCR — ou option « OCR par vision » via `mistral-small-3.2-24b` (multimodal) : rendu pages→images + transcription chat.
- ✅ **Sélecteur de modèle dans le chat** : menu déroulant dans le composeur (`#chatModelSelect`) **synchronisé** avec celui du rail (« Modèle ») via `setModel()` — change le modèle interrogé sans aller dans le rail. — **fait**.
